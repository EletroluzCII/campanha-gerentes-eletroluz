...
create table public.metric_submissions (
  id uuid primary key default gen_random_uuid(),
...
create index metric_submissions_branch_created_idx
  on public.metric_submissions(branch_id, created_at desc, id desc);

insert into public.branches (slug, name, display_order) values
...
create or replace function public.has_campaign_profile()
returns boolean
language sql
stable
security definer
...
create policy "authenticated users read active branches"
  ...
  using (is_active and public.has_campaign_profile());

create policy "users read their own profile or admins read all"
  on public.profiles for select to authenticated
  using (id = auth.uid() or public.is_campaign_admin());

create policy "managers read own history and admins read all"
...
as $$
begin
  if not public.has_campaign_profile() then
...
create or replace function public.get_admin_latest_metrics()
returns table (
  branch_id uuid,
  branch_name text,
...
begin
  ...
  select distinct on (ms.branch_id)
    ...
    b.name,
    ms.obz_percentage,
    ms.revenue_percentage,
    ms.discount_band,
    ms.discount_percentage,
    ms.development_points,
    ms.total_points,
    ms.created_at
  from public.metric_submissions ms
...
revoke all on public.metric_submissions from anon;
revoke insert, update, delete, truncate, references, trigger on public.branches from authenticated;
revoke insert, update, delete, truncate, references, trigger on public.profiles from authenticated;
revoke insert, update, delete, truncate, references, trigger on public.metric_submissions from authenticated;

grant select on public.branches to authenticated;
grant select on public.profiles to authenticated;
grant select on public.metric_submissions to authenticated;

revoke all on function public.is_campaign_admin() from public;
revoke all on function public.has_campaign_profile() from public;
revoke all on function public.submit_metrics(numeric, numeric, text, numeric, boolean, boolean, boolean, boolean) from public;
revoke all on function public.get_campaign_ranking() from public;
revoke all on function public.get_admin_latest_metrics() from public;

grant execute on function public.is_campaign_admin() to authenticated;
grant execute on function public.has_campaign_profile() to authenticated;
grant execute on function public.submit_metrics(numeric, numeric, text, numeric, boolean, boolean, boolean, boolean) to authenticated;
grant execute on function public.get_campaign_ranking() to authenticated;
grant execute on function public.get_admin_latest_metrics() to authenticated;
